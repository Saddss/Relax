...
**交付物**：一份 `RESULT.md`，格式见 §5。§4.3 的脚本会自动生成其中大部分内容。
...
**唯一交付物是一个 markdown 文件 `RESULT.md`**，跑完后连同（若有问题的）原始日志一起发回。§4.3 的 `collect.sh` 已经生成了第 1–4 节，你只需要补第 5–7 节（人工判断部分）。
...
以下任一成立，请把对应的**完整**日志一起发回，不要只发 `RESULT.md`：
...
```bash
# 跑完四组后
bash collect.sh > RESULT.md
# 补完第 5-7 节，然后
ls -la RESULT.md /tmp/rloo-*.log
```
